import styled from 'styled-components'

const PostIcon = ({ className }) => {
  return (
    <svg
      className={className}
      width='24'
      height='24'
      viewBox='0 0 24 24'
      fill='none'
      xmlns='http://www.w3.org/2000/svg'
    >
      <path
        d='M22.88 15.1204L15.25 22.7504L11.5 23.5004L12.25 19.7504L19.88 12.1204C20.0763 11.9237 20.3095 11.7677 20.5662 11.6612C20.8229 11.5548 21.0981 11.5 21.376 11.5C21.6539 11.5 21.9291 11.5548 22.1858 11.6612C22.4425 11.7677 22.6757 11.9237 22.872 12.1204L22.88 12.1284C23.0767 12.3247 23.2327 12.5579 23.3391 12.8146C23.4456 13.0713 23.5004 13.3465 23.5004 13.6244C23.5004 13.9022 23.4456 14.1774 23.3391 14.4341C23.2327 14.6908 23.0767 14.924 22.88 15.1204Z'
        stroke='white'
        strokeLinecap='round'
        strokeLinejoin='round'
      />
      <path
        d='M8.5 23.5H1.5C1.23478 23.5 0.98043 23.3946 0.792893 23.2071C0.605357 23.0196 0.5 22.7652 0.5 22.5V1.5C0.5 1.23478 0.605357 0.98043 0.792893 0.792893C0.98043 0.605357 1.23478 0.5 1.5 0.5H14.793C15.0582 0.500057 15.3125 0.605451 15.5 0.793L19.207 4.5C19.3946 4.68749 19.4999 4.9418 19.5 5.207V8.5'
        stroke='white'
        strokeLinecap='round'
        strokeLinejoin='round'
      />
    </svg>
  )
}

const Styled = styled(PostIcon)``
Styled.displayName = 'PostIcon'
export default Styled
